...
tiedot = tiedot.drop_duplicates(subset=["LVI-koodi", "Yleisnimi", "Tekninen nimi"])
...
# Tarkistetaan, onko tietovarasto jo olemassa
if not os.path.exists(chromadb_polku):
    print("Tietovarastoa ei löytynyt, luodaan uusi...")
    documents = [
        Document(
            page_content=f"{row['LVI-koodi']} {row['Yleisnimi']} {row['Tekninen nimi']}",
            metadata={
                "LVI-koodi": row["LVI-koodi"],
                "Yleisnimi": row["Yleisnimi"],
                "Tekninen nimi": row["Tekninen nimi"]
            }
        )
    ...
    try:
        ...
        print(f"Virhe tietovaraston luomisessa: {e}")
else:
    ...
    try:
        ...
        print("Tietovarasto ladattu onnistuneesti.")
    except Exception as e:
        print(f"Virhe tietovaraston lataamisessa: {e}")
...
def prosessoi_teksti_gpt(kysely):
    try:
        ...
        response = openai.chat.completions.create(
            ...
            messages=[  # Viestit
                {"role": "system", "content": "Olet avustaja, joka osaa jakaa tekstin avainsanoihin, korjata kirjoitusvirheet ja tulkita synonyymejä."},
                {"role": "user", "content": f"Jaa seuraava teksti avainsanoihin ja vertaa niitä annettuun tuotteet.xlsx tiedostoon. Palauta niiden tuotteiden tiedot jotka tekstissä mainitaan edes lähes samassa muodossa tai synonyymina. Ota huomioon myös taivutusmuodot: {kysely}"}
            ]
        )
